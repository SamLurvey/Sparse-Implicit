...
def evaluate_model(model, original_data, transformed_data, max_iters, nBins, sample_fraction):
    start_time = time.time()
    
    # Train the model on transformed data
    mse_transformed = model.train_entropy(transformed_data, total_steps = max_iters,nBins=nBins,sample_fraction=sample_fraction)
...
def evaluate_models(hyperparameters, original_samples, transformed_samples, file_names, model_classes):
    ...
    for i, (original_data, transformed_data, file_name) in enumerate(zip(original_samples, transformed_samples, file_names)):
        ...
        for ModelClass in model_classes:
            ...
            mse_original, mse_transformed, compression_ratio, training_time = evaluate_model(model, original_data, transformed_data, hyperparameters["max_iters"], hyperparameters["nBins"], hyperparameters["sample_fraction"])
