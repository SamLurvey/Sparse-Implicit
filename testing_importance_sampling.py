...
def evaluate_model(model, original_data, transformed_data, max_iters, sample_fraction):
    start_time = time.time()
    
    # Train the model on transformed data
    mse_transformed = model.train_importance(transformed_data, total_steps=max_iters, sample_fraction=sample_fraction)
...
def evaluate_models(hyperparameters, original_samples, transformed_samples, file_names, model_classes):
    ...
    for i, (original_data, transformed_data, file_name) in enumerate(zip(original_samples, transformed_samples, file_names)):
        ...
        for j in range(20):
            print(f"\nEvaluating on data sample {i+1} {j}th time: {file_name}")
            ...
            for ModelClass in model_classes:
                ...
                mse_original, mse_transformed, compression_ratio, training_time = evaluate_model(model, original_data, transformed_data, hyperparameters["max_iters"], hyperparameters["sample_fraction"])
...
def hyperparameter_grid_search(base_grid, model_grids):
    ...
    # Save results to CSV
    results_df.to_csv('SIREN_same_importance_comparison_results.csv', index=False)
